...
This branch has the CountDown Timer implementation, the Timer is launched on App startup and used to update the local database with remote data when it is done counting down. It also caters for the period the App is not launched or is in the background.
